 ...
 * weekly hypertrophy board (see {@link cardioLegDose} for the shared dose model).
 * The cardio category itself stays off the board — only the lower body is worked.
 */
function creditCardioLegs(set: WorkSet, credit: (muscle: MuscleGroup, amount: number) => void): void {
  const dose = cardioLegDose(set);
  if (dose <= 0) return;
  for (const { muscle, share } of CARDIO_LEG_MUSCLES) credit(muscle, share * dose);
}
