...
- `claudeRoutine.ts` + `views/claudeRoutine.ts`: `buildRoutinePrompt()` is the history-driven cousin of `buildPlanPrompt` — it feeds Claude the logged history + current 1RM for ONE selectable compound lift and asks for a periodised, set-based block (a `kind:"session"` `RoutineSheet` with explicit reps×load per set) tuned to a `RoutineGoal` (strength / hypertrophy / 1RM peak). Handed off via `startRoutineInClaude`/`copyRoutinePrompt`; the reply pastes back through the same `parsePlanFromText`. Reached from the Stats per-exercise view (a compound lift seeds it via `seedRoutineKey`).
