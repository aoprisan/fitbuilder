...
| **Train** | `views/live.ts` | The live logger — a state machine (`Stage` = list/select/exercise, `SetSub` = idle/running/logging/resting) with stopwatch, rest timer + alert, the concentric-ring exercise picker (`views/radial.ts`: muscle groups / that muscle's exercises / recent movements, confirmed on the hub — `views/live.ts` still renders the classic chip lists under the Picker toggle, saved by `pickerLayout.ts`), rotary kg/reps dial (`views/dial.ts`), RIR chips, warm-up ramp, plate calculator, PR flash, superset jumps, and per-exercise plan benchmark (`benchmark.ts`) for sessions that carried a routine prescription. |
...
- Other localStorage keys: `gymlog.oneRm` (user-tested maxes), `gymlog.bodyweight` (set in Settings; feeds protein/calorie estimates), rest-alert, picker-layout (`gymlog.pickerLayout`) and theme/language prefs. `backup.ts` bundles/restores every raw `gymlog.*` entry — it's schema-agnostic on purpose.
...
`src/dom.ts` (`h()` element builder, `append()`, `clear()`, `qs()`), `src/util.ts` (uuid, formatters, serializers), `src/i18n.ts` (EN/RO — every view registers its own strings via `registerTranslations` at module load and reads `t()` at mount time), `src/theme.ts` (4 themes as CSS custom-property sets), `src/styles.css` (the entire "vintage letterpress / Training Ledger" design system — tokens, hard block-print shadows, print styles). Reusable view widgets: `views/dial.ts`, `views/radial.ts` (concentric ring picker), `views/chart.ts`, `views/sessionSummary.ts` (effort/hydration/protein panel shared by Train, History and exports), `views/filter.ts`, `views/snackbar.ts` (undo), `views/lookback.ts`.
