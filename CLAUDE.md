...
| **Train** | `views/live.ts` | The live logger — a state machine (`Stage` = list/select/exercise, `SetSub` = idle/running/logging/resting) with stopwatch, rest timer + alert, the concentric-ring exercise picker (`views/radial.ts`: muscle groups / that muscle's exercises / recent movements, confirmed on the hub — the only exercise picker; picks repaint the dial in place rather than re-rendering the screen, so it never scrolls out from under the thumb), rotary kg/reps dial (`views/dial.ts`) with one-tap reps × load picks (plan / last set / last time / progression) and a live volume·e1RM read-out beside it, RIR chips, warm-up ramp, plate calculator, PR flash, superset jumps, and per-exercise plan benchmark (`benchmark.ts`) for sessions that carried a routine prescription. |
